package com.git.learning;

public class Calculator {// in project name firstone is projectName second is repo name 3 master branch
	
	// this is like we join the company and company has nothing we start this brand new
	// project from beginning.
	
	// things we need // they will send you link for joining GithUB
	//1- login credentials for GitHub.(user_name + password)  goto GitHub.come enter userName+password sign up/sign in
	//

	
	public int add(int x,int y) {
		int result = 0;
		result = x+y;
		return result;
	}
	
	public int multiply(int x,int y) {
		return x*y;
	}
}
